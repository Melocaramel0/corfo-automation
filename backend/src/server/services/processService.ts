...
export class ProcessService {
  ...
  async executeProcessWithMonitoring(processId: string): Promise<string> {
    ...
    // Iniciar ejecución en background (no esperar respuesta) - ejecución paralela
  ...
  private async runAgenteOrquestadorInBackground(
    ...
    try {
      ...
      // Inyectar URL del formulario, executionId y executionService para actualización de progreso
      (agente as any).formUrl = process.rutaFormulario;
      (agente as any).executionId = executionId;
      (agente as any).executionService = executionService;
      (agente as any).shouldLogToExecution = true; // Activar logs a executionService
  ...
  private async captureLogs(executionId: string, executeFunc: () => Promise<void>): Promise<void> {
    // NO interceptar console.log - el AgenteOrquestador usa su logger interno
    // que guarda logs en executionService solo para su executionId específico
    try {
      await executeFunc();
    } catch (error) {
      console.error(`❌ Error durante la ejecución del Agente Orquestador:`, error);
      throw error;
    }
  }
  ...
  private async saveExecutionResult(executionId: string, processId: string, resultado: ResultadoAgente): Promise<boolean> {
    try {
      ...
      // Solo si el resultado tiene datos válidos (pasos procesados o estadísticas)
      const tieneDatosValidos = 
        (resultado.pasosCompletados && resultado.pasosCompletados.length > 0) ||
        (resultado.estadisticas && resultado.estadisticas.totalCampos > 0) ||
  ...
  async getProcessLogs(processId: string): Promise<any[]> {
    ...
    // Los logs ya vienen estructurados desde executionService, solo filtrar por processId
    const executionLogs = executions.flatMap(exec => 
      exec.logs.filter(log => log.procesoId === processId)
    );

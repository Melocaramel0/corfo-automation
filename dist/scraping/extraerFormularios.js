...
exports.CAMPOS_CORFO_MAPPING = void 0;
exports.extraerPrimerFormulario = extraerPrimerFormulario;
exports.extraerFormularios = extraerFormularios;
exports.extraerPrimerFormularioMultiFase = extraerPrimerFormularioMultiFase;
exports.analizarFormularioCompleto = analizarFormularioCompleto;
...
async function login(page) {
    ...
    try {
        ...
        await loginFrame.evaluate(() => {
            const btn = document.getElementById('ingresa_');
            if (btn)
                btn.click();
        });
 ...
 */
async function extraerCamposFormulario(page) {
 ...
 */
async function extraerPrimerFormulario() {
 ...
 */
async function extraerFormularios() {
 ...
 */
async function extraerFormularioMultiFase(page) {
        ...
        if (hayMasFases) {
            ...
            try {
                ...
                try {
                    ...
                    console.log(`❌ Error al hacer clic: ${clickError.message}`);
 ...
 */
async function extraerCamposFase(page, numeroFase) {
    const campos = [];
    try {
        ...
        if (campos.length === 0) {
            ...
            const elementosFlexibles = elements.filter(el => el.debug.isDisplayed && // Solo que esté display != none
 ...
 */
async function detectarCamposObligatorios(page, campos) {
    console.log('🔍 Detectando campos obligatorios...');
    try {
        ...
        if (!botonSiguiente) {
            ...
            try {
                ...
                console.log('⚠️ Error al hacer clic en botón, posible navegación:', error.message);
                return [];
            }
        }
        ...
        camposConError.forEach(campo => {
            if (campo) {
                ...
                const campoCoincidente = campos.find(c => c.nombre === campo.name ||
                    c.nombre === campo.id ||
 ...
 */
async function llenarCamposMinimos(page, camposObligatorios) {
 ...
 */
async function extraerPrimerFormularioMultiFase() {
    // Inicializar el navegador
    const browser = await test_1.chromium.launch({
        headless: false // Mantenemos visible para ver el proceso
    });
    ...
    if (!primerMasInfoLink) {
        ...
        for (const enlace of enlaces) {
            try {
                ...
                if (texto && (texto.toLowerCase().includes('información') ||
    ...
    if (!primerMasInfoLink) {
        ...
        for (const enlace of enlacesFinales) {
            try {
                const texto = await enlace.textContent();
                const href = await enlace.getAttribute('href');
                if (texto && href && (texto.toLowerCase().includes('información') ||
 ...
 */
async function navegarAConvocatorias(page) {
    console.log('Navegando a la página de convocatorias...');
    try {
 ...
 */
async function accederAlFormulario(page) {
    ...
    if (!botonIniciarDirecto) {
        ...
        for (const enlace of todosLosEnlaces) {
            try {
                const texto = await enlace.textContent();
                if (texto && (texto.toLowerCase().includes('inicia tu postulacion') ||
        ...
        try {
            ...
            if (page.url().includes('PostuladorBorradores.aspx')) {
                ...
                try {
                    ...
                    if (tablaPostulaciones) {
                        ...
                        if (iconoPapelera) {
                            ...
                            try {
                                ...
                                console.log('❌ Error al eliminar postulación previa:', error.message);
                            }
                        }
                        else {
                    ...
                    console.log('⚠️ Error al verificar postulaciones previas:', error.message);
                }
            ...
            console.log('❌ Error al hacer clic en botón directo:', error.message);
        }
    }
    ...
    if (!formularioAccedido) {
        ...
        for (let i = 0; i < frames.length; i++) {
            try {
                ...
                console.log(`   Error revisando frame ${i + 1}:`, error.message);
            }
        }
    }
 ...
 */
async function analizarFormularioCompleto() {
 ...
 */
async function analizarTodosLosPasos(page) {
    const pasos = [];
    let pasoActual = 1;
 ...
 */
async function obtenerNombrePasoActual(page) {
    try {
        ...
        console.log('⚠️ Error al obtener nombre del paso:', error.message);
 ...
 */
async function buscarYExpandirSecciones(page) {
    ...
    for (const elemento of elementosExpandibles) {
        try {
            ...
            const esExpandible = dataToggle.includes('collapse') ||
                className.includes('accordion') ||
                className.includes('collapse') ||
                className.includes('toggle') ||
                ariaExpanded !== null ||
                (texto && (texto.includes('RECOMENDACIONES') ||
 ...
 */
async function extraerCamposDirectos(page) {
 ...
 */
async function extraerCamposDeSeccionExpandida(page, elementoExpandido) {
...
async function manejarModalConfirmacion(page) {
    try {
        ...
        console.log(`   ⚠️ Error al manejar modal: ${error.message}`);
        return false;
    }
}
/**
 ...
 */
function calcularCamposPorTipo(pasos) {
    const contadores = {};
 ...
 */
function generarResumenAnalisis(pasos, totalCampos, camposPorTipo) {
 ...
 */
async function guardarReporteTexto(analisis) {
